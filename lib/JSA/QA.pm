...
sub analyse_timeseries_rms {
    ...
    if (! defined($mmm_return->{min}) ||
        ...
        $result->pass(0);
        my $fail_reason = sprintf(
            "Receptor-to-receptor RMS values varied by more than %d%%\n",
            int($rmsvar_rcp * 100));
        $fail_reason .= sprintf(
            "  [min=%.2f (-%.2f%% of mean) max=%.2f (+%.2f%% of mean) mean=%.2f]",
            $mmm_return->{min},
    ...
    if (! $result->pass && $iterate) {
        ...
        if ($numgood <= $goodrecep) {
            ...
            if( $result->pass ) {
                ...
                my $note = "Receptor-to-receptor RMS value test passed after removing receptor"
                         . (scalar(@receptors_removed) > 1 ? 's ' : ' ') . join ',', sort @receptors_removed;
                $result->add_note($note);
            }
            else {
                my $fail_reason = sprintf(
                    "Receptor-to-receptor RMS values varied by more than %d%% after removing high-RMS receptors.\n",
                    int($rmsvar_rcp * 100));
                $fail_reason .= sprintf(
                    "  [min=%.2f (-%.2f%% of mean) max=%.2f (+%.2f%% of mean) mean=%.2f]\n",
                    $mmm2->{min},
...
sub analyse_tsysvar {
    ...
    if (! defined( $mmm_return->{min} ) ||
        ...
        $result->pass(0);
        my $fail_reason = sprintf(
            "Receptor-to-receptor Tsys values varied by more than %d%%\n",
            int($tsysvar * 100));
        $fail_reason .= sprintf(
            "  [min=%.2f (-%.2f%% of mean) max=%.2f (+%.2f%% of mean) mean=%.2f]",
            $mmm_return->{min},
    ...
    if (defined($mmm_return->{mean}) &&
        ...
        $result->pass(0);
        my $fail_reason = sprintf(
            "Mean Tsys value (%.2f) is greater than maximum allowed value (%d)",
            $mmm_return->{mean},
            $tsysmax);
        $result->add_fail_reason($fail_reason);
    }
    ...
    if (! $result->pass && $iterate) {
        ...
        if ($numgood <= $goodrecep) {
            ...
            if ($result->pass) {
                $result->clear_fail_reasons;
                my $note =
                    "Receptor-to-receptor Tsys value test passed after removing receptor"
                    . (scalar(@receptors_removed) > 1 ? 's ' : ' ')
                    . (join ',', sort @receptors_removed) . ".";
                $result->add_note($note);
            }
            else {
                my $fail_reason =
                    "Receptor-to-receptor Tsys value test still failed after removing receptor"
                    . (scalar(@receptors_removed) > 1 ? 's ' : ' ')
                    . (join ',', sort @receptors_removed) . ".";
                $result->add_fail_reason($fail_reason);
            }
